{
	...
	//trigger	
	string beforecutMC = "abs(Tracker.q_inn-1)<0.3 && (Tof.z_nhit>=3)&&(Tof.z_like>0)&&((Tof.clsn[0]+Tof.clsn[2])<=4) && Tof.beta>0&&(trigpatt&0x2)!=0";
	string aftercutMC  = "abs(Tracker.q_inn-1)<0.3 && (Tof.z_nhit>=3)&&(Tof.z_like>0)&&((Tof.clsn[0]+Tof.clsn[2])<=4) && Tof.beta>0&&(trigpatt&0x2)!=0&&(Header.sublvl1&0x1E)!=0";		
	string beforecutDT  = "Header.pres_weight*(100*(abs(Tracker.q_inn-1)<0.3 && (Tof.z_nhit>=3)&&(Tof.z_like>0)&&((Tof.clsn[0]+Tof.clsn[2])<=4) && Tof.beta>0&&((trigpatt&0x2)!=0)&&(Header.sublvl1&0x3E)==0))";		
	string aftercutDT   = "Header.pres_weight*((abs(Tracker.q_inn-1)<0.3 && (Tof.z_nhit>=3)&&(Tof.z_like>0)&&((Tof.clsn[0]+Tof.clsn[2])<=4) && Tof.beta>0&&((trigpatt&0x2)!=0)&&(Header.sublvl1&0x3E)!=0))";
	...
	TH1F * hbeforeMC = new TH1F("hbeforeMC","hbeforeMC",100,-1,2);
	TH1F * hafterMC   = new TH1F("hafterMC","hafterMC",100,-1,2) ; 
	TH1F * hbeforeDT = new TH1F("hbeforeDT","hbeforeDT",100,-1,2);
	TH1F * hafterDT   = new TH1F("hafterDT","hafterDT",100,-1,2) ; 
