{
	...
	gROOT->ProcessLine(".L ~/Work/Deutons/DirectAnalysis/InputFileReader.h");
	...
	//trigger	
/*	string beforecutMC =                           "abs(Tracker.q_inn-1)<0.3 &&Tof.beta>0&&Tracker.q_lay[1][0]>0&&((trigpatt&0x2)!=0)";
	string aftercutMC  =                           "abs(Tracker.q_inn-1)<0.3 &&Tof.beta>0&&Tracker.q_lay[1][0]>0&&((trigpatt&0x2)!=0)&&(Header.sublvl1&0x3E)!=0";		
	string beforecutDT  = "Header.pres_weight*(100*(abs(Tracker.q_inn-1)<0.3 &&Tof.beta>0&&Tracker.q_lay[1][0]>0&&((trigpatt&0x2)!=0)&&(Header.sublvl1&0x3E)==0) || (abs(Tracker.q_inn-1)<0.3 &&Tof.beta>0&&Tracker.q_lay[1][0]>0&&((trigpatt&0x2)!=0)&&(Header.sublvl1&0x3E)!=0))";		
	string aftercutDT   = "Header.pres_weight*(    (abs(Tracker.q_inn-1)<0.3 &&Tof.beta>0&&Tracker.q_lay[1][0]>0&&((trigpatt&0x2)!=0)&&(Header.sublvl1&0x3E)!=0))";
*/	
	//tracking
	...
	string beforecutDT = "Header.pres_weight*((Tof.z_nhit>=3)&&(Tof.z_like>0)&&((Tof.clsn[0]+Tof.clsn[2])<=4) && Tof.beta>0&&(trigpatt&0x2)!=0&&(Header.sublvl1&0x3E)!=0 && SA.beta>0 && SA.beta_patt==0)";
	string aftercutDT  = "Header.pres_weight*((Tof.z_nhit>=3)&&(Tof.z_like>0)&&((Tof.clsn[0]+Tof.clsn[2])<=4) && Tof.beta>0&&(trigpatt&0x2)!=0&&(Header.sublvl1&0x3E)!=0 && SA.beta>0 && SA.beta_patt==0 && Tracker.rig[1]!=0)";//&&Tracker.q_lay_status[1][1]==0";		
...
//	string aftercutDT   = "Header.pres_weight*(abs(Tracker.q_inn-1)<0.3 && (trigpatt&0x2)!=0 &&(Header.sublvl1&0x1E)!=0 && Tracker.q_lay[1][0] > 0 && Tracker.rig[1]>0&& Tof.beta>0&& (Tof.z_nhit>=3)&&(Tof.z_like>0)&&((Tof.clsn[0]+Tof.clsn[2])<=4))";
	...
	chainMC->Draw("log10(rig[1])>>hafterMC",beforecutMC.c_str());
	chainDT->Draw("log10(rig[1])>>hbeforeDT",beforecutDT.c_str());
	chainDT->Draw("log10(rig[1])>>hafterDT",beforecutDT.c_str());
