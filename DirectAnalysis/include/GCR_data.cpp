...
int GCR_data::read(char* database_file_, char *area_units_, char *energy_units_)
{
  ...
  for(j=0;j<n;j++)
    {
      ...
      //cout<<i<<endl;
     sscanf(entry[j],"%c%s%s%s%s%s%le%le%le%le%le%le%s%d.%d%d.%d%d.%d |%d.%d%d.%d%d.%d%40c",
                     &status[i],reference[i],experiment[i],X_units[i],Y_units[i],Y_name[i],
     ...
     strcpy(entry[i],entry[j]);               // OK since j>=i;
     //    if(strstr(&status[i],"#")==NULL){ i++ ; }// eliminate # commented data. caused problems, changed to strncmp as for errtype below AWS20121119
    if(strncmp(&status[i],"#",1)!=0){ i++ ; }// eliminate # commented data  changed to strncmp as for errtype below AWS20121119
...
int GCR_data::set_plotting()
{
 ...
 // so define the k.. values explicitly.
  ...
  color[i]=kBlack;                                          
  style[i]=kFullCircle;                                   
  size [i]=0.5; //1.0;                                                                       //AWS20090303

  if(strncmp(experiment[i],"AMS01"   ,5)==0){ color[i]=kCyan   ;style[i]=kOpenCircle;  }     //AWS20090227
  ...
  if(strncmp(experiment[i],"CAPRICE" ,7)==0){ color[i]=kGreen;  style[i]=kOpenStar  ;  }     //AWS20090227
  ...
  if(strncmp(experiment[i],"HESS09"  ,6)==0){ color[i]=kGreen;  style[i]=kFullSquare    ;  size[i] = 1.0;   } //AWS20090521
  ...
  if(strncmp(experiment[i],"CREAM"   ,5)==0){ color[i]=kRed  ;  style[i]=kOpenDiamond   ; size[i] = 1.0;  } //AWS20090602
  if(strncmp(experiment[i],"ISOMAX"  ,6)==0){ color[i]=kRed  ;  style[i]=kFullTriangleUp; size[i] = 1.0;  } //AWS20091109

  if(strncmp(experiment[i],"Fermi-LAT",8)==0){color[i]=kRed  ;  style[i]=kFullCircle    ; size[i] = 0.6;  } //AWS20090306 AWS20090915
