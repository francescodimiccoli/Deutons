     ...
     //TOF
      ...
      if(cmask.isFromNaF()) {//NaF
	    ...
	    if(Massa_gen<1&&Massa_gen>0.5) ((TH2*) FitNaF_Dbins -> TemplateP) -> Fill(mass,Kbin,Tup.mcweight);
            if(Massa_gen<2&&Massa_gen>1.5) for(int mctype=0;mctype<6;mctype++) ((TH3*)FitNaF_Dbins -> TemplateD) -> Fill(mass,Kbin,mctype,Tup.mcweight);
      	 ...
      	 Kbin=NaFPB.GetBin(RUsed);   
            ...
            if(Massa_gen<2&&Massa_gen>1.5) for(int mctype=0;mctype<6;mctype++) ((TH3*)FitNaF_Pbins -> TemplateD) -> Fill(mass,Kbin,mctype,Tup.mcweight);
   ...
   for(int j=0;j<5;j++)
	   for(int i=0;i<5;i++){
		   ...
		   if(frac!="tot"){
			   ...
			   FitTOF_Dbins[i][j]       ->  SetFitConstraints(ContaminationTOF,0.6,1,0.0001,0.2);
			   FitNaF_Dbins[i][j]       ->  SetFitConstraints(ContaminationNaF,0.6,1,0.0001,0.2);
			   FitAgl_Dbins[i][j]       ->  SetFitConstraints(ContaminationAgl,0.6,1,0.0001,0.2);
		   }
