...

{
   ...
   for(int i=0; i<1/*ntupla2->GetEntries()/fraz*/; i++) {
      ...
      if(((int)Cutmask>>10)&1==1) IsPrescaled=1;
      else IsPrescaled=0;
         ...
         if((DistTOF<4&&DistTrack<4)||IsHe) {
            for(int m=0; m<17; m++)
               if(fabs(R)<bin[m+1]&&fabs(R)>bin[m]) {
                  if(IsHe) Discr2=4*Massa;
                  if(Massagen<1) {
                     Discr1_TMCP->Fill(Discr1,m);
                     Discr2_TMCP->Fill(Discr2,m);
                  }
                  if(Massagen<2&&Massagen>1) {
                     Discr1_TMCD->Fill(Discr1,m);
                     Discr2_TMCD->Fill(Discr2,m);
                  }
                  if(Massagen<4&&Massagen>2) {
                     Discr1_TMCHe->Fill(Discr1,m);
   ...
   selectedQHL3->Write();
   selectedQHL4->Write();
   selezioniQHLMC1->Write();
   selezioniQHLMC2->Write();
   selezioniQHLMC3->Write();
   selezioniQHLMC4->Write();
   selectedQHLMC1->Write();
   selectedQHLMC2->Write();
   selectedQHLMC3->Write();
   selectedQHLMC4->Write();
   selezioniQHL1_P->Write();
   selezioniQHL2_P->Write();
   selezioniQHL3_P->Write();
   selezioniQHL4_P->Write();
   selectedQHL1_P->Write();
   selectedQHL2_P->Write();
   selectedQHL3_P->Write();
   selectedQHL4_P->Write();
   selezioniQHLMC1_P->Write();
   selezioniQHLMC2_P->Write();
   selezioniQHLMC3_P->Write();
   selezioniQHLMC4_P->Write();
   selectedQHLMC1_P->Write();
   selectedQHLMC2_P->Write();
   selectedQHLMC3_P->Write();
   selectedQHLMC4_P->Write();
