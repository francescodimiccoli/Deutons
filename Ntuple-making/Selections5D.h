...
bool Quality(TTree *albero,int i)
{
...

	{
		...
		if(R!=0) var[2]=(fabs(Rup-Rdown)/R); else var[2]=1;
		var[3]=layernonusati;
		var[4]=fuoriX;
		var[5]=Chisquare;
		var[6]=TOF_Up_Down;
...
void Nuovasel(float RG,float M, TF1 *RBETA){
	...
	for(int z=0;z<1e6;z++){
		...
		if(Dist<Dist1) { DR1=0;Dist1=Dist; CooTOF[0]=distETOFU; CooTOF[1]=distB; CooTOF[2]=distR; 
			CooTrack[0]=distETrack; CooTrack[1]=distB; CooTrack[2]=distR; 
			CooTRD[0]=distETOFD; CooTRD[1]=distB; CooTRD[2]=distR;
			Rmin=RGDT;}
		else DR1++;
		if(!((((int)Cutmask)>>11)==512||(((int)Cutmask)>>11)==0)){
			if(DR1>25) break;
	...
	RminTOF=Rmin; RminTrack=Rmin; RminTRD=Rmin;		
...
float CalculateLklhd()
{
	...
	PhiProjTOF=acos(PSCALTOF2/DistTOF);if(PSCALTOF3<0) PhiProjTOF=2*pi-PhiProjTOF;
	PhiProjTrack=acos(PSCALTrack2/DistTrack);if(PSCALTrack3<0) PhiProjTrack=2*pi-PhiProjTrack;
	PhiProjTRD=acos(PSCALTRD2/DistTRD);if(PSCALTRD3<0) PhiProjTRD=2*pi-PhiProjTRD;
	std::cout<<"////"<<std::endl;
	...
	if(DistTOF<10&&DistTrack<10&&DistTRD<10&&PhiProjTOF<10&&PhiProjTrack<10&&PhiProjTRD<10&&RminTOF<3.8&&RminTrack<3.8&&RminTRD<3.8){
		for(int k=0;k<11;k++){
			Par=0.5+k*0.3;
			if(RminTOF>Par&&RminTOF<=Par+0.3) {  logL_true+=log10((*TOF_Phi)[(int)(PhiProjTOF*10)][k])+log10((*TOF_Dist)[(int)(DistTOF*10)][k]);
		...
		for(int k=0;k<11;k++){
			Par=0.5+k*0.3;
			if(RminTrack>Par&&RminTrack<=Par+0.3) {  logL_true+=log10((*Track_Phi)[(int)(PhiProjTrack*10)][k])+log10((*Track_Dist)[(int)(DistTrack*10)][k]);
		...
		for(int k=0;k<11;k++){
			Par=0.5+k*0.3;
			if(RminTRD>Par&&RminTRD<=Par+0.3) {  logL_true+=log10((*TRD_Phi)[(int)(PhiProjTRD*10)][k])+log10((*TRD_Dist)[(int)(DistTRD*10)][k]);
