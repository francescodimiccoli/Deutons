...
int main()
{
	...
	for(int z=0;z<11;z++){
		...
		graficoz[6][z]=new TH1F(("TOF Up-Down"+to_string(z)).c_str(),(Variables[6]+"_"+to_string(z)).c_str(),200,0,100);
		graficoz[7][z]=new TH1F(("Track Up-Down"+to_string(z)).c_str(),(Variables[7]+"_"+to_string(z)).c_str(),300,0,30);	    
	...
	TH1F *Sl =new TH1F("Signal","Signal",25,0,1);
	TH1F *Bd =new TH1F("Bkgnd","Bkgnd",25,0,1);
	TH1F *He =new TH1F("Bkgnd","Bkgnd",25,0,1);
	TH1F *LKL_Data=new TH1F("Lk_Data","Signal",25,0,1);
	TH1F *LKL_Dataz[11];
	...
	for(int l=0; l<ntupla1->GetEntries()/3;l++) {
		...
		if(Beta<0.75&&Beta>0&&(1/Massa)<MassLimit&&R>0&&R<4&&IsCharge1==1)
		{
	...
	for(int l=0; l<ntupla2->GetEntries()/3;l++) {
		...
		if(IsCharge1==1&&R>1.2*Rcutoff&&R>0&&R>30){
                        ...
                        graficoz[6][zone]->Fill(TOF_Up_Down);
                        graficoz[7][zone]->Fill(Track_Up_Down);
		...
		float L_Discr=(L_true/(L_false+L_true));
		if(IsCharge1==1&&R>1.2*Rcutoff&&R>30){
