...
int main()
{
	...
	for(int i=0; i<ntupla1->GetEntries();i++) {
		...
		if(((int)Cutmask>>11)==512&& Beta<0.97&&Beta>0.8&&(1/Massa)<MassLimit&&Massagen<1&&R>0&&R<9) {
                                RvsRgen_bad->Fill(R,Momentogen);
                                if(R<100){
                                sigmagen_bad->Fill(fabs(R-Momentogen)/(pow(Momentogen,2)*Rig->Eval(Momentogen)),fabs(Beta-Betagen)/(pow(Beta,2)*betaNaF->Eval(Beta)));
                                }
                }
