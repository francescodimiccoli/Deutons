...
float SimulateBadEvents(int EvNum,float Beta,int Level){
	if(EvNum%Level==0) { float beta= Rand->Uniform(0.8,1); return beta;}
	return Beta; 
...
int TailTestNaF(){
	...
	vars->ReadAnalysisBranches(treeDT);
        ...
        for(int i=0;i<treeDT->GetEntries()/frac;i++){
                ...
                treeDT->GetEvent(i);
		if(ControlSample(vars)&&IsFromNaF(vars)&&vars->BetaRICH_new<0.98&&LikelihoodCut(vars)&&DistanceCut(vars))
	...
	int passed=0;
        for(int i=0;i<treeMC->GetEntries()/frac;i++){
                ...
                treeMC->GetEvent(i);
		if(IsFromNaF(vars)&&IsProtonMC(vars))passed++;
		if(ControlSample(vars)&&IsFromNaF(vars)&&vars->BetaRICH_new<0.98&&IsProtonMC(vars)&&LikelihoodCut(vars)&&DistanceCut(vars)){
		...
		if(IsFromNaF(vars)&&IsProtonMC(vars))  { betasmear1=SimulateBadEvents(passed,vars->BetaRICH_new,80);
							 betasmear2=SimulateBadEvents(passed,vars->BetaRICH_new,50);
							 betasmear3=SimulateBadEvents(passed,vars->BetaRICH_new,15);
							}			
		if(ControlSample(vars)&&IsFromNaF(vars)&&betasmear1<0.98&&IsProtonMC(vars)&&LikelihoodCut(vars)&&DistanceCut(vars)){
			MassMCSmear1->Fill(vars->R/betasmear1*pow(1-pow(betasmear1,2),0.5),vars->mcweight);
		}	
		if(ControlSample(vars)&&IsFromNaF(vars)&&betasmear2<0.98&&IsProtonMC(vars)&&LikelihoodCut(vars)&&DistanceCut(vars)){
			MassMCSmear2->Fill(vars->R/betasmear2*pow(1-pow(betasmear2,2),0.5),vars->mcweight);
		}	
		if(ControlSample(vars)&&IsFromNaF(vars)&&betasmear3<0.98&&IsProtonMC(vars)&&LikelihoodCut(vars)&&DistanceCut(vars)){
